...
//        val maxHeightPx = with(density) { maxHeight.toPx() }.toInt()
//
//        // LaunchedEffect para recalcular cuando el texto, estilo, límites de fuente o el tamaño del contenedor cambien
//        LaunchedEffect(text, style, minFontSize, maxFontSizeLimit, maxWidthPx, maxHeightPx) {
//            readyToDraw = false
//            var bestFitFontSize = minFontSize // Empezamos asumiendo el mínimo
//
//            // Asegurarnos de que los límites para la búsqueda sean válidos
...
//            var upperBoundSp = maxFontSizeLimit.value.coerceAtLeast(minFontSize.value)
//
//            if (lowerBoundSp > upperBoundSp + 0.01f) { // Rango inválido (e.g., min > max limit)
//                currentFontSize = minFontSize
//                readyToDraw = true
//                return@LaunchedEffect
//            }
//
//            // 1. Comprobar si el texto con minFontSize ya se desborda
//            val minFontLayoutResult = textMeasurer.measure(
//                text = AnnotatedString(text),
...
//
//            // 2. Búsqueda binaria para encontrar el tamaño de fuente más grande que quepa.
//            // Iteramos un número fijo de veces, lo cual es común para búsquedas binarias en UI
//            // para asegurar convergencia sin bucles infinitos por precisión de floats.
//            // Unas 10-15 iteraciones suelen ser suficientes para sp.
//            repeat(15) {
//                if (upperBoundSp - lowerBoundSp < 0.1f) { // Si la diferencia es muy pequeña, hemos convergido
//                    currentFontSize = bestFitFontSize
//                    readyToDraw = true
//                    return@LaunchedEffect
//                }
...
//                val candidateFontSize = midSp.sp
//
//                // Evitar medir tamaños más pequeños que nuestro mejor ajuste conocido
//                if (candidateFontSize.value < bestFitFontSize.value) {
//                    lowerBoundSp = midSp + 0.01f // Continuar búsqueda en la mitad superior
//                    return@repeat // Saltar esta iteración de repeat
//                }
//
//                val layoutResult = textMeasurer.measure(
//                    text = AnnotatedString(text),
//                    style = style.copy(fontSize = candidateFontSize),
//                    overflow = TextOverflow.Clip,
//                    softWrap = true,
//                    maxLines = Int.MAX_VALUE,
//                    constraints = Constraints(
...
//            Text(
//                text = text,
//                // El modifier del Text en sí no necesita fillMaxSize aquí,
//                // ya que el tamaño de la fuente se calcula para caber en el BoxWithConstraints.
//                // Si quieres que el área táctil del Text llene el Box, podrías usar Modifier.fillMaxSize()
//                modifier = Modifier,
//                style = style.copy(fontSize = currentFontSize),
//                overflow = TextOverflow.Ellipsis, // Trunca si, a pesar de todo, aún se desborda
//                softWrap = true,
//                maxLines = Int.MAX_VALUE // El tamaño de fuente se eligió para que todas las líneas quepan en altura
//            )
//        }
//    }
//}