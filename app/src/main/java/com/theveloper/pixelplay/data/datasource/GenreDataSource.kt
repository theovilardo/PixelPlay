...
//object GenreDataSource {
//
//    // Función auxiliar para calcular la luminancia percibida de un color.
//    // Se utiliza para decidir si el 'on-color' debe ser claro (para base oscura) o oscuro (para base clara).
//    private fun getLuminance(color: Int): Double {
//        val r = Color.red(color)
//        val g = Color.green(color)
//        val b = Color.blue(color)
//        // Fórmula estándar WCAG para la luminancia relativa (sRGB)
//        val rLinear = if (r / 255.0 <= 0.03928) (r / 255.0) / 12.92 else Math.pow((r / 255.0 + 0.055) / 1.055, 2.4)
//        val gLinear = if (g / 255.0 <= 0.03928) (g / 255.0) / 12.92 else Math.pow((g / 255.0 + 0.055) / 1.055, 2.4)
//        val bLinear = if (b / 255.0 <= 0.03928) (b / 255.0) / 12.92 else Math.pow((b / 255.0 + 0.055) / 1.055, 2.4)
//        return 0.2126 * rLinear + 0.7152 * gLinear + 0.0722 * bLinear
//    }
//
//    /**
//     * Genera un color 'on-color' tintado para un color base dado.
//     * El 'on-color' tendrá un contraste suficiente (blanco o negro como base)
//     * y un sutil tinte del color original.
//     *
//     * @param baseColorHex El color hexadecimal del fondo (ej. "#FF4081").
//     * @return El color hexadecimal del 'on-color' tintado.
//     */
//    private fun getTintedOnColorHex(baseColorHex: String): String {
//        val baseColor = Color.parseColor(baseColorHex)
//        val baseLuminance = getLuminance(baseColor)
//
//        // Decide si el color 'on' principal debe ser blanco o negro, basado en la luminancia del color base.
//        // Un umbral de 0.5 (luminancia relativa) es un punto de partida común para distinguir entre claro y oscuro.
//        val primaryOnColor = if (baseLuminance > 0.5) Color.BLACK else Color.WHITE
//
//        // Factor de mezcla: qué porcentaje del tinte del color original se añade al 'on-color' principal.
//        // Un valor más pequeño significa menos tinte, más puro blanco/negro. Un valor más grande significa más tinte.
//        // Este valor debe elegirse cuidadosamente para mantener el contraste y que el tinte sea perceptible.
//        val blendFactor = 0.15f // 15% del tinte original. Puedes ajustar este valor (ej. 0.1f para menos, 0.2f para más).
//
//        // Calcular los componentes de color mezclados usando interpolación lineal (LERP)
//        val blendedR = (Color.red(primaryOnColor) * (1 - blendFactor) + Color.red(baseColor) * blendFactor).roundToInt().coerceIn(0, 255)
//        val blendedG = (Color.green(primaryOnColor) * (1 - blendFactor) + Color.green(baseColor) * blendFactor).roundToInt().coerceIn(0, 255)
//        val blendedB = (Color.blue(primaryOnColor) * (1 - blendFactor) + Color.blue(baseColor) * blendFactor).roundToInt().coerceIn(0, 255)
//
//        // Convertir los componentes RGB mezclados de nuevo a una cadena hexadecimal
//        return String.format("#%02X%02X%02X", blendedR, blendedG, blendedB)
//    }
//
//    val staticGenres = listOf(
//        Genre(id = "pop", name = "Pop", colorHex = "#FF4081", onColorHex = getTintedOnColorHex("#FF4081")), // Color base: Rosa vibrante. On-color: Negro con un tinte rosa.
//        Genre(id = "rock", name = "Rock", colorHex = "#D32F2F", onColorHex = getTintedOnColorHex("#D32F2F")), // Color base: Rojo fuerte. On-color: Blanco con un tinte rojo.
//        Genre(id = "hip_hop", name = "Hip Hop", colorHex = "#7B1FA2", onColorHex = getTintedOnColorHex("#7B1FA2")), // Color base: Morado profundo. On-color: Blanco con un tinte morado.
//        Genre(id = "electronic", name = "Electronic", colorHex = "#00BCD4", onColorHex = getTintedOnColorHex("#00BCD4")), // Color base: Cyan. On-color: Negro con un tinte cyan.
//        Genre(id = "classical", name = "Classical", colorHex = "#C8E6C9", onColorHex = getTintedOnColorHex("#C8E6C9")), // Color base: Verde suave. On-color: Negro con un tinte verde.
//        Genre(id = "jazz", name = "Jazz", colorHex = "#FFC107", onColorHex = getTintedOnColorHex("#FFC107")), // Color base: Ámbar. On-color: Negro con un tinte ámbar.
//        Genre(id = "reggae", name = "Reggae", colorHex = "#4CAF50", onColorHex = getTintedOnColorHex("#4CAF50")), // Color base: Verde. On-color: Negro con un tinte verde.
//        Genre(id = "blues", name = "Blues", colorHex = "#1976D2", onColorHex = getTintedOnColorHex("#1976D2")), // Color base: Azul. On-color: Blanco con un tinte azul.
//        Genre(id = "metal", name = "Metal", colorHex = "#424242", onColorHex = getTintedOnColorHex("#424242")), // Color base: Gris oscuro. On-color: Blanco con un tinte gris oscuro.
//        Genre(id = "folk_acoustic", name = "Folk & Acoustic", colorHex = "#A1887F", onColorHex = getTintedOnColorHex("#A1887F")), // Color base: Marrón claro. On-color: Negro con un tinte marrón.
//        Genre(id = "rnb_soul", name = "R&B / Soul", colorHex = "#E91E63", onColorHex = getTintedOnColorHex("#E91E63")), // Color base: Rosa. On-color: Blanco con un tinte rosa.
//        Genre(id = "latin", name = "Latin", colorHex = "#FF9800", onColorHex = getTintedOnColorHex("#FF9800")) // Color base: Naranja. On-color: Negro con un tinte naranja.
//    )
//}
